...
w = int(0.5*fs) # длина одной линии = ширина кадра
h = len(data_am)//w # высота кадра
image = Image.new('RGB', (w, h))

px, py = 0, 0 # счетчики пикселей по длине и ширине кадра
for p in range(0, len(data_am), 1):
    lum = int(data_am[p]//32-32) # определение цвета пиксля
    if lum < 0: lum = 0
    if lum > 255: lum = 255
    image.putpixel((px, py), (0, lum, 0))
    px += 1
    if px >= w:
        if (py % 50) == 0:
            print(f"Line saved {py} of {h}") # просто чтобы отслеживать процесс загрузки
        px = 0
        py += 1
        if py >= h:
            break
